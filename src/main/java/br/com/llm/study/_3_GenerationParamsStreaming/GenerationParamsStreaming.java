package br.com.llm.study._3_GenerationParamsStreaming;

import br.com.llm.study._2_ConversationState.ConversationState;
import br.com.llm.study.dtos.ChatCompletionChunk;
...
public class GenerationParamsStreaming {
    ...
    static void main() {
        Message systemMessage = new Message("system", "You are a helpful tech assistent. Ask as many question as you want to know the customer wants. Always be polite ");
        ...
        while (!isOver) {
            try {
                ...
                ChatCompletionRequest chatCompletionRequest = new ChatCompletionRequest(
                        "gpt-5.1", messages, 0.7, 300, true, responseFormat);
                ...
                final HttpResponse<Stream<String>> response = httpClient.send(request, HttpResponse.BodyHandlers.ofLines());
                ...
                response.body().forEach(line -> {
                    if (!line.startsWith("data: ")) return;          // skip blank lines
                    String json = line.substring(6);
                    if (json.equals("[DONE]")) return;               // stream sentinel
                    try {
                        var chunk = OBJECT_MAPPER.readValue(json, ChatCompletionChunk.class);
                        var choice = chunk.choices().get(0);
                        String piece = choice.delta().content();
                        if (piece != null) {
                            System.out.print(piece);   // print tokens as they stream in
                            System.out.flush();
                            full.append(piece);
                        }
                ...
                ConversationState.UserInput userInput = OBJECT_MAPPER.readValue(full.toString(), ConversationState.UserInput.class);
